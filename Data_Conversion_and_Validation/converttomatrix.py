...
def process_all_games():
    ...
    game_files = glob.glob("data/game_moves_small.csv")
    ...
    try:
        ...
        # Convert move-level data to NumPy arrays with the new structure
        X_boards = np.array([d['board_state'] for d in all_data])
        y_start = np.array([d['move_features']['start_pos'] for d in all_data])
        y_end = np.array([d['move_features']['end_pos'] for d in all_data])
        y_direction = np.array([d['move_features']['direction'] for d in all_data])
        y_distance = np.array([d['move_features']['distance'] for d in all_data])
        y_legal_moves = np.array([d['move_features']['legal_moves'] for d in all_data])
        y_winner = np.array([d['winner'] for d in all_data])

        # Save dataset with enhanced features
        output_file = "hnefatafl_dataset_small.npz"
        ...
        for move in all_data:
            ...
            game_summaries[game_id]['final_position'] = move['board_state']
            if move.get('had_capture', False):
                game_summaries[game_id]['captures'] += 1
